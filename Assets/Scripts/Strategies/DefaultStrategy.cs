...
/**
	<summary>
	DefaultStrategy implements the default calculation methods for balancing the game
	as close as possible to the original Flash version of Parameters.
	</summary>
*/
public class DefaultStrategy : MonoBehaviour, IParametersStrategy {
	...
	public int CalculateDamageBy(Room attacker) {
		...
		float defense = Player.instance.defense;
		if (defense > 300) { defense = 300; } // maximum defense for player.

		damage = damage * 0.5f * (1- (defense/300) ); // calculate damage
	...
	public int CalculateNextLevelEXP() {
		int exp = 100 + ( Player.instance.level - 1 ) * ( Player.instance.level - 1 );
		return exp;
	}
	...
	public float CalculatePlayerRecoveryStat(Player.PlayerStats stat) {
		...
		switch (stat) {
			case Player.PlayerStats.Life:
				recovery = (Player.instance.lifeRecoverMin + Player.instance.recovery * Player.instance.lifeRecoverRate) * Time.deltaTime;
				break;

			case Player.PlayerStats.Attack:
				recovery = (Player.instance.attackRecoverMin + Player.instance.recovery * Player.instance.attackRecoverRate) * Time.deltaTime;
				break;

			case Player.PlayerStats.Defense:
				recovery = (Player.instance.defenseRecoverMin + Player.instance.recovery * Player.instance.defenseRecoverRate) * Time.deltaTime;
				break;

			case Player.PlayerStats.Action:
				recovery = (Player.instance.actionRecoverMin + Player.instance.recovery * Player.instance.actionRecoverRate) * Time.deltaTime;
				break;
		}
	...
	public int CalculateMissionGrindReward(Mission mission) {
		int reward =  (int)((mission.width * mission.height / 10) + UnityEngine.Random.value * mission.width * mission.height / 100) / 10;
		return reward;
	}
	...
	public int CalculateMissionProgress(Mission mission) {
		int progress = (int) (mission.progressMin + UnityEngine.Random.value * mission.progressRange);
		return progress;
	}
